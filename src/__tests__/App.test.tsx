...
describe("App 组件", () => {
  ...
  it("应该渲染工具箱导航并默认打开计算器", () => {
    ...
    expect(
      within(navigation).getByRole("button", { name: "计算器" }),
  ...
  it("应该支持在导航页面之间切换", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "梦幻新诛仙实用工具" }),
    ).toBeInTheDocument();
  ...
  it("应该保存三个角色存档并一键恢复角色面板和装备配置", async () => {
    ...
    expect(profileNotice).toHaveTextContent("已保存“鬼王69”的完整角色配置");
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole("combobox", { name: "角色等级" })).toHaveValue(
        "69",
      );
  ...
  it("应该支持搜索和筛选门派资料", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "南疆古巫" }),
    ).toBeInTheDocument();
  ...
  it("应该保存装备、角色等级、宝石和独立词条并在重新加载后恢复", async () => {
    ...
    await waitFor(() => {
      ...
      expect(storedState.equipment.weapon.baseAttributes.physicalAttack).toBe(
        700,
      );
  ...
  it("应该把 v1 装备缓存迁移到包含角色等级的 v2 状态", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "编辑武器" }));
    expect(
    ...
    await waitFor(() => {
      ...
      expect(migratedState.equipment.weapon.baseAttributes.physicalAttack).toBe(
        701,
      );
    });
  });
